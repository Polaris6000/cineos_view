 ...
 *
 * 상태 관리:
 *   messages, isSending — AiChatContext에서 관리 (페이지 이동 후에도 대화 내용 유지)
 *   inputText           — 로컬 state (입력 중인 텍스트는 이동 시 초기화해도 무방)
 *
 * Props:
 *   isOpen        — 패널 열림 여부 (입력창 포커스 타이밍 용도)
...
function AiChatPanel({isOpen, conversationId, onClose}: AiChatPanelProps) {
  // messages, isSending: Context에서 가져옴 → 페이지 이동해도 내용 유지
  const {messages, setMessages, isSending, setIsSending} = useAiChat()
  
  // 입력창 텍스트는 로컬 state로 관리 (이동 시 초기화해도 무방)
   ...
   *
   * isSending이 true인 상태에서 페이지를 이동해도 fetch는 계속 실행되고,
   * 완료 시 Context의 messages/isSending이 업데이트된다.
   */
  ...
  }, [inputText, isSending, conversationId, setMessages, setIsSending])
