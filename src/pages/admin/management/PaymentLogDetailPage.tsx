...
function InfoRow({ label, value, highlight }: { label: string; value: React.ReactNode; highlight?: boolean }) {
  return (
    <div style={infoRow}>
      ...
      <span style={{ ...infoValue, fontWeight: highlight ? 800 : 500, color: highlight ? 'var(--color-brand-default)' : 'var(--text-primary)' }}>
        {value}
      </span>
    </div>
  )
...
function PaymentLogDetailPage() {
  ...
  useEffect(() => {
    ...
    const fetchDetail = async () => {
      ...
      setError('')
      try {
        const { data } = await apiClient.get<PaymentDTO>(`/payment/read/${id}`)
        setRaw(data)
  ...
  return (
    <div style={{ maxWidth: 640 }}>
      ...
      <div style={idBox}>
        ...
        <p style={{ fontFamily: 'monospace', fontSize: 15, fontWeight: 700, color: 'var(--text-primary)', margin: 0, wordBreak: 'break-all' }}>
          {detail.bookingId}
        </p>
      </div>
      ...
      <div style={card}>
        ...
        {detail.pointUsed > 0 && (
          <InfoRow
            label="포인트 사용"
            value={<span style={{ color: 'var(--color-success-main)' }}>−{detail.pointUsed.toLocaleString()}P</span>}
          />
        )}
