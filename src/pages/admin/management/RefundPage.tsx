 ...
 *
 * ⚠️ GET /api/payment/list 엔드포인트는 백엔드 팀이 추가 예정.
 *    현재는 엔드포인트가 없으므로 리스트 영역에 로딩 실패 안내 표시됨.
 */
      ...
      <section style={sectionCard}>
        ...
        {result && (
          <div style={{ marginTop: 20 }}>
            ...
            <dl style={dl}>
              <dt style={dtStyle}>예매번호</dt>  <dd style={ddStyle}>{result.bookingId}</dd>
              <dt style={dtStyle}>휴대폰</dt>    <dd style={ddStyle}>{result.phone}</dd>
              <dt style={dtStyle}>영화</dt>      <dd style={ddStyle}>{result.movieTitle}</dd>
              <dt style={dtStyle}>상영관</dt>    <dd style={ddStyle}>{result.theaterName}</dd>
              <dt style={dtStyle}>일시</dt>      <dd style={ddStyle}>{result.date} {result.startTime}</dd>
              <dt style={dtStyle}>좌석</dt>      <dd style={ddStyle}>{result.seats.join(', ')}</dd>
              <dt style={dtStyle}>결제금액</dt>
              <dd style={{ ...ddStyle, fontWeight: 700 }}>
                {result.totalAmount.toLocaleString()}원
              </dd>
              <dt style={dtStyle}>결제수단</dt>  <dd style={ddStyle}>{result.paymentMethod}</dd>
              <dt style={dtStyle}>결제일시</dt>  <dd style={ddStyle}>{result.paidAt.replace('T', ' ')}</dd>
              {result.pointUsed > 0 && (
                <>
