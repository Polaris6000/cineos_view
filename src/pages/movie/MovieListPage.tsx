...
function MovieListPage() {
  ...
  // length === 0만으로는 "아직 로딩 중"과 "스케줄 없음"을 구분할 수 없어서 별도 플래그 관리
  ...
  useEffect(() => {
    const axiosTheater = async () => {
      try {
        ...
        setTheater(data.map((dto) => mapToTheater(dto)))
      } catch (error) {
  ...
  /**
   * 예매 마감 여부 확인 — 상영 시작 후 10분까지는 예매 가능
   ...
   * schedulesLoaded가 false이면 로딩 중으로 간주해 일단 true 반환 (깜박임 방지)
   * movieId는 Number()로 강제 변환 — 백엔드가 문자열로 내려줄 경우 === 비교 실패 방지
   */
  const hasRemainingToday = (movieId: number): boolean => {
    if (!schedulesLoaded) return true
    const id = Number(movieId)
    const todaySchedules = schedules.filter(s => Number(s.movieId) === id && s.date === today)
